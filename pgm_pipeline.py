...
class PGM_Pipeline(object):
  ...
  def run_qiime_report(self):
    run_QiimeReport_command = "python " + os.path.join(script_path, "qiime_report_j1.py") + " -i " + self.output + "/trimmed_fastq/all_reads.fna -o " + self.output + " -m " + self.map + " -n " + str(self.num_cpus)
