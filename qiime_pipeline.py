...
class Pipeline(object):
  ...
  def get_params(self):
    letters = 'i:,o:,m:,c:,l:,q:,r:,s:,h'
    keywords = ['input-dir=', 'output-dir=', 'map=', 'cores=','length=', 'quality=', 'reference=', 'sort=', 'help']
    ...
    if self.input is None or self.output is None or self.map is None or self.quality is None or self.length is None:
  ...
  def run_pipeline(self):
    try:
      #os.system("./bam_to_fastq.py -- -i " + self.input + " -o " + self.output + " -m " + self.map)
      #os.system("./trim_primers.py -- -i " + self.output + "/fastq/ -o " + self.output + "/trimmed_fastq/ -l " + self.length + " -q " + self.quality + " -m " + self.map)
      #run_qiime_string = "./run_qiime.py -- -i " + self.output + "/trimmed_fastq/all_reads.fna -o " + self.output + " -m " + self.map + " -c " + str(self.cores) + " -s " + self.sort
  ...
  def print_help():
    print '''
    qiipe_pipeline.py runs the qiime pipeline for 16s metagenomic analysis on given input data. 
    Parameters:
    (-i, --input-dir) The directory containing all the .bam files to be analyzed.
    ...
    (-m, --map) The tab-delimited qiime mapping file that contains metadata for the samples. Please read the manual for specifications on this file
      The manual is located in /mnt/software/qiime/my_scripts/qiime_pipeline/documentation
    (-c, --cores) The number of cores to use for the parallel portions of this pipeline. Default is 1. 
    (-q, --quality) The quality threshold for reads. All reads below this quality will be removed before analysis is performed.
    (-l , --length) the minimum length for reads. All reads shorter than this length will be removed before analysis is performed. 
  ...
  def generate_HTML(self):
    ...
    os.system("cp /mnt/software/qiime/qiime_pipeline/Workflow.png " + outputDir + "/report_files/Workflow.png")
